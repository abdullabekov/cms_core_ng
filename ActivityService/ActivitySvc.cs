...
namespace ActivityService
{
    public class ActivitySvc : IActivitySvc
    {
        ...
        public async Task AddUserActivity(ActivityModel model)
        {
            ...
            {
                Log.Error("An error occured while AddUserActivity {Error} {StackTrace} {InnerException} {Source}",
        ...
        public async Task<List<ActivityModel>> GetUserActivity(string userId)
        {
            ...
            {
                Log.Error("An error occured while GetUserActivity {Error} {StackTrace} {InnerException} {Source}",
                      ex.Message, ex.StackTrace, ex.InnerException, ex.Source);
                await dbContextTransaction.RollbackAsync();
            }
